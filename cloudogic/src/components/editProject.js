...
function AddProject({ projectData }) {
    ...
    // inputValues.pName = projectData.pName;
    // inputValues.pDescription = projectData.pDescription;
    ...
    function register() {
        ...
        axios.put('http://localhost:4000/edit/' + projectData.pId, inputValues)
            .then(response => {
                setSuccessMessage("Project Modified successfully ...")
                setSuccessStatus(true)
                setErrorStatus(false)
            }).catch(error => {
                setErrorMessage("Project modification failed...!")
                setErrorStatus(true)
                setSuccessStatus(false)
            })
    }
    function handleSubmit(event) {
        ...
        if(inputValues.pName=="") inputValues.pName=projectData.pName;
        if(inputValues.pDescription=="") inputValues.pDescription=projectData.pDescription;
        register();
    ...
    const checkValidation = () => {
        ...
        if (!inputValues.noOfMembers.trim() || inputValues.noOfMembers==="select") {
            errors.noOfMembers = "select no. of members";
        } else {
            errors.noOfMembers = "";
        }

        if (!validation.skillSet.trim() && !validation.noOfMembers.trim()) {
            setbtnStatus(false)
        ...
        <div >
            {backButton ? <Home /> :
                <div className='addForm  shadow-lg p-3 mb-5 bg-white rounded'>
                    <h2 className="form-group"> Modify Project : {projectData.pName}</h2>
                    <form onSubmit={handleSubmit}>
                        <div className="form-group">
                            <input type="text" className="form-control" defaultValue={projectData.pName} onChange={handleChange} name="pName" id="pName" aria-describedby="emailHelp" placeholder="Project Name" />
                        </div>
                        <div className="form-group">
                            <input type="text" className="form-control" defaultValue={projectData.pDescription} onChange={handleChange} name='pDescription' id="pDescription" placeholder="Project Description" />
                        </div>
